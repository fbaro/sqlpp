...
public class TreeLayoutTest {
    ...
    private static Tree xCommaY(String x, String y) {
        return r -> r
                .child("", ",", ch2 -> ch2.leaf(x))
                .child("", "", ch2 -> ch2.leaf(y));
    }

    private static Tree xEqualY(String x, String y) {
        return r -> r
                .child("", "", ch2 -> ch2.leaf(x))
                .child("=", "", ch2 -> ch2.leaf(y));
    }

    private Tree simpleCode = c -> c
            .child("SELECT", "", xCommaY("A", "B"))
            .child("FROM", "", ch -> ch.leaf("TABLE"));

    private Tree longTablesCode = c -> c
            .child("SELECT", "", xCommaY("A", "B"))
            .child("FROM", "", xCommaY("VERYLONGTABLE1", "VERYLONGTABLE2"));

    private Tree innerJoinCode = c -> c
